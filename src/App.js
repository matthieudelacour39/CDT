...
// ── JSONBin config ────────────────────────────────────────────
const API_KEY = "$2a$10$wOlesJ4oRDKBQxvZFpZdKeY8AKTEpPCGhUOp6SK.mTX0RipLpXEju";
const BIN_NAME = "cahier-de-texte";
let BIN_ID = localStorage.getItem("jsonbin-id") || null;
...
async function jbSave(data) {
  if (!BIN_ID) {
    ...
    if (!r.ok) throw new Error("Création bin échouée");
    const j = await r.json();
    BIN_ID = j.metadata.id;
    localStorage.setItem("jsonbin-id", BIN_ID);
  } else {
...
export default function CahierDeTexte() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await jbLoad();
        if (data) {
          if (data.classes) setClasses(data.classes);
          if (data.entries) setEntries(data.entries);
          if (data.eleves) setEleves(data.eleves);
          // Synchroniser le BIN_ID sur cet appareil si besoin
          if (data.binId && !localStorage.getItem("jsonbin-id")) {
            BIN_ID = data.binId;
            localStorage.setItem("jsonbin-id", BIN_ID);
          }
        }
  ...
  // ── Sauvegarde avec debounce (évite trop d'appels API) ──────
  const saveAll = useCallback((c, en, el) => {
    setSaving(true);
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(async () => {
      try {
        await jbSave({ classes: c, entries: en, eleves: el, binId: BIN_ID });
        // Garder aussi en localStorage comme cache
        localStorage.setItem("cahier-classes", JSON.stringify(c));
        localStorage.setItem("cahier-entries", JSON.stringify(en));
        localStorage.setItem("cahier-eleves", JSON.stringify(el));
      } catch(e) {
